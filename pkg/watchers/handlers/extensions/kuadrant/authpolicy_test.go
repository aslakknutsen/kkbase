...
// TestAuthPolicyFieldRequirements_KnownVersions validates that our field requirements
// work against known CRD schema versions at build time
//
// CRD fixtures are from official Kuadrant releases:
// - authpolicy-v1-crd.yaml: Kuadrant v1.3.0
func TestAuthPolicyFieldRequirements_KnownVersions(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			version:       "v1",
			crdFile:       "authpolicy-v1-crd.yaml",
			shouldSupport: true,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			extractor, err := schema.NewFieldExtractor(versionSchema, tt.version, AuthPolicyFieldRequirements)
