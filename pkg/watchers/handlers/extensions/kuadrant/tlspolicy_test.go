...
// TestTLSPolicyFieldRequirements_KnownVersions validates that our field requirements
// work against known CRD schema versions at build time
//
// CRD fixtures are from official Kuadrant releases:
// - tlspolicy-v1-crd.yaml: Kuadrant v1.3.0
func TestTLSPolicyFieldRequirements_KnownVersions(t *testing.T) {
	// Register CRD types
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			extractor, err := schema.NewFieldExtractor(versionSchema, tt.version, TLSPolicyFieldRequirements)
