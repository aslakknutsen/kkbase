...
// TestRateLimitPolicyFieldRequirements_KnownVersions validates that our field requirements
// work against known CRD schema versions at build time
//
// CRD fixtures are from official Kuadrant releases:
// - ratelimitpolicy-v1-crd.yaml: Kuadrant v1.3.0
func TestRateLimitPolicyFieldRequirements_KnownVersions(t *testing.T) {
	// Register CRD types
	...
	tests := []struct {
	...
	}{
		{
			...
			version:       "v1",
			crdFile:       "ratelimitpolicy-v1-crd.yaml",
			shouldSupport: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			extractor, err := schema.NewFieldExtractor(versionSchema, tt.version, RateLimitPolicyFieldRequirements)
