...
// TestKuadrantFieldRequirements_KnownVersions validates that our field requirements
// work against known CRD schema versions at build time
//
// CRD fixtures are from official Kuadrant releases:
// - kuadrant-v1-crd.yaml: Kuadrant v1.3.0 (uses v1beta1)
func TestKuadrantFieldRequirements_KnownVersions(t *testing.T) {
	// Register CRD types
	...
	tests := []struct {
		...
		{
			name:          "v1beta1 - full support",
			version:       "v1beta1",
			crdFile:       "kuadrant-v1-crd.yaml",
			shouldSupport: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Note: Kuadrant CR has minimal spec, so we pass empty requirements if none are defined
			extractor, err := schema.NewFieldExtractor(versionSchema, tt.version, KuadrantFieldRequirements)
			...
			// Kuadrant CR has minimal spec, so no required fields to check
			_ = extractor // Mark as used
		})
	}
}
